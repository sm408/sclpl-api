...
@pytest.mark.parametrize(
    ("relative_path", "denied", "match"),
    [
        pytest.param(("outputs", "report.csv"), False, None, id="underneath-the-root"),
        pytest.param(("elsewhere", "report.csv"), True, "outside", id="sibling-path"),
        pytest.param(
            ("outputs", "..", "..", "escaped.csv"), True, None, id="path-traversal-walks-back-out"
        ),
    ],
)
def test_output_root_accepts_only_paths_underneath_it(
    tmp_path: Path, relative_path: tuple[str, ...], denied: bool, match: str | None
) -> None:
    (tmp_path / "outputs").mkdir()
    resolved = policy.parse(_project(tmp_path, "[policy]\noutput_roots = ['outputs']\n"))
    candidate = tmp_path.joinpath(*relative_path)
    if not denied:
        resolved.check_output(candidate)  # does not raise
        return
    with pytest.raises(PolicyDenied, match=match):
        resolved.check_output(candidate)
...
@pytest.mark.parametrize(
    ("policy_toml", "match"),
    [
        pytest.param("nonsense = true", "unknown policy key", id="unknown-key"),
        pytest.param("hosts = 'api.example.com'", "policy.hosts", id="hosts-not-an-array"),
